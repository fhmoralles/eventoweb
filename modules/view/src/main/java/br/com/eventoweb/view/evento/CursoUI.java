...
@SessionScoped
@LoggedIn
public class CursoUI extends AbstractCRUD<Atividade, AtividadeFilter> {
	...
	//private static final Logger LOG = Logger.getLogger(CursoUI.class);
	
	private static final String CURSO_PESQUISAR = "/forms/eventos/cursoPesquisar.xhtml";
	private static final String CURSO_CADASTRAR = "/forms/eventos/cursoCadastrar.xhtml";
	...
	protected Atividade newInstance() {
		...
		curso.setParticipante(new Participante());
		curso.getParticipante().setEvento(this.getFilter().getEvento());
		curso.setTipoAtividade(TipoAtividade.CURSO);
		curso.getParticipante().setTipoParticipante(TipoParticipante.PALESTRANTE);
	...
	protected List<Atividade> searchImpl(AtividadeFilter filter) {
		return atividadeModel.cursosEvento(filter.getEvento());
	}
